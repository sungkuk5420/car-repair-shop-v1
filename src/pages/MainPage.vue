      ...
      <q-carousel-slide :name="1" class="column no-wrap">
        <div class="scroll-container">
          <MainContent v-for="(conetent, index) in mainContents" :key="index" :data="conetent"
            @handleClick="handleClickContent"></MainContent>
        </div>
      </q-carousel-slide>
        ...
        <div class="write-form">
          ...
          <div class="write-form__submit-row">
            ...
            <div class="write-form__submit-row__right">
              <q-btn class="col-12" flat @click="addComment">등록하기</q-btn>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async asyncComments () {
      ...
      const thisObj = this;
      thisObj.comments = [];
      const querySnapshot = await getDocs(collection(this.db, this.currentContent.id + "Comments"));
      querySnapshot.forEach((doc) => {
        thisObj.comments.push(doc.data())
      });
      thisObj.comments = thisObj.comments.sort(function (x, y) {
        return x.index - y.index;
      })
    },
    async handleClickContent (currentContent) {

      this.currentContent = currentContent;
      console.log(this.currentContent.id)
    ...
    async addComment () {
      ...
      if (!this.comment) {
        this.errorMessage("한마디를 입력해주세요.")
        return false
      }
      if (!this.loginUser) {
        this.errorMessage("로그인 후 등록가능합니다.")
        return false
      }

      const time = new Date().toLocaleString({ timeZone: 'Asia/Tokyo' });
      const result = await addDoc(collection(this.db, this.currentContent.id + "Comments",), {
